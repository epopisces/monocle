...
- Updated `docs/build-plan.md` and `docs/milestones.md` to archive M43 and advance the active milestone to M44
- Validation: `uv run python -m pytest monocle/tests/test_api.py -k "fast_capture or ingest_review" monocle/tests/test_ingest_prepare.py -x --tb=short -q` → 13 passed, 76 deselected, EXIT 0; `uv run python -m pytest monocle/tests/test_ingest_sessions.py -k "update_proposed_action" -x --tb=short -q` → 1 passed, 8 deselected, EXIT 0; `uv run python -m pytest monocle/tests/test_ingest.py monocle/tests/test_ingest_prepare.py monocle/tests/test_api.py -x --tb=short -q` → 164 passed, EXIT 0
- Completed M44 `Unified Capture Workbench Backend Contract`: added `monocle/services/capture_workbench.py`, `monocle/routers/capture_workbench.py`, shared response models, and `IngestSessionStore.count_sessions()` so prepared sessions, queue-threshold-filtered pending notes, failed sessions, and failed-ingest records are exposed through one aggregated backend contract
- Rewired legacy `/api/review`, `/api/review/count`, and `/api/ingest/failures` routes onto shared capture-workbench helpers, added focused API coverage in `monocle/tests/test_api.py`, and regenerated `openapi.json` plus `frontend/src/api/schema.d.ts`
- Validation: `uv run python -m pytest monocle/tests/test_api.py -k "capture_workbench" -x --tb=short -q` → 2 passed, 83 deselected, EXIT 0; `uv run python -m pytest monocle/tests/test_api.py monocle/tests/test_review.py monocle/tests/test_ingest.py -x --tb=short -q` → 201 passed, EXIT 0; `cd frontend && npx tsc --noEmit` → EXIT 0
- Completed M45 `Unified Capture Workbench Frontend`: added a new `CaptureWorkbench` drawer and typed `/api/capture-workbench` client, replaced the app-shell’s separate prepared/review/failure polling path with one summary query, and collapsed the topbar to one actionable workbench badge while keeping `/ingest-review` and `/docs` as deep workflow handoffs
- Updated `App.test.tsx`, `CaptureWorkbench.test.tsx`, and `VoiceCapture.test.tsx` for the unified workbench affordance, and strengthened the `react-force-graph` test stub so the full frontend suite stays stable when Vitest shares a worker between files
- Validation: `cd frontend && npm run test -- --run` → 473 passed, EXIT 0; `cd frontend && npx tsc --noEmit` → EXIT 0
- Completed M46 `Lean Chat & Explicit URL Capture`: removed the hidden `/api/chat` `fetch_urls` prefetch path, changed the chat composer to surface detected links only through an explicit `Capture URLs` ingest handoff, and kept `create_reference_from_url` available through explicit agent/MCP tool use with generic chat tool-call UI
- Refreshed the generated contracts by exporting `openapi.json` and regenerating `frontend/src/api/schema.d.ts` from the local OpenAPI file after the standard frontend gen-api task failed due no server running on `localhost:8000`
- Validation: `cd frontend && npm run test -- --run src/Chat.test.tsx src/useChat.test.ts --reporter=dot` → 80 passed, EXIT 0; `cd frontend && npx tsc --noEmit` → EXIT 0; `uv run python -m pytest monocle/tests/test_agents.py monocle/tests/test_mcp.py -x --tb=short -q` → 112 passed, EXIT 0
- Completed M47 `Omnisearch Hardening`: replaced `/api/search/omni`'s repeated full-vault scan with the new `monocle/services/omni_search.py` cached catalog, wired per-file invalidation through the reindex callback, and extended the focused omni API coverage for trimmed query length, people frontmatter matches, cache reuse, and cache refresh behavior
- Hardened the topbar omnisearch component in `frontend/src/components/layout/OmniSearch.tsx` so it trims the fast-match query, ignores stale slower responses, keeps `Ctrl+E` from hijacking other form fields, blurs on `Escape`, and fixes grouped option IDs to match `aria-activedescendant`
- Validation: `uv run python -m pytest monocle/tests/test_api.py::TestOmniSearch -x --tb=short -q` → 11 passed, EXIT 0; `cd frontend && npm run test -- --run src/OmniSearch.test.tsx --reporter=dot` → 15 passed, EXIT 0; `cd frontend && npx tsc --noEmit` → EXIT 0
- Completed M48 `Cleanup & Contract Hardening`: removed the retired `ReviewQueue`, `FailedCaptures`, and `IngestInbox` UI/test surfaces; deleted obsolete review-count, ingest-notification, and failed-ingest-list adapter routes and client wrappers; and kept only the mutation paths still used by the unified capture workbench
- Refreshed `openapi.json` and `frontend/src/api/schema.d.ts`, then updated `docs/srs.md`, `docs/build-plan.md`, and `docs/milestones.md` so the unified capture workbench, explicit URL capture, distinct omnisearch, and unified runtime are the only documented mainline architecture
- Validation: `uv run python -m pytest monocle/tests/ -x --tb=short -q` → 942 passed, 8 deselected, EXIT 0; `cd frontend && npm run test -- --run` → 432 passed, EXIT 0; `cd frontend && npx tsc --noEmit` → EXIT 0
- Addressed the post-M48 review findings: bounded explicit chat URL capture to the first 5 distinct URLs, switched the chat-origin ingest handoff from concurrent fan-out to sequential requests, and added focused chat regressions for capped capture behavior
- Hardened the capture workbench failure section so failed ingest records and failed sessions are merged by newest timestamp before the section limit is applied, and added API coverage for mixed failure ordering under `limit_per_section`
- Aligned the frontend chat SSE contract with the backend `done.status` payload, stopped unfinished tool calls from being marked `success` on error completions, and added regressions for the error-completion path plus the no-action ingest review state transition
- Validation: `cd frontend && npm run test -- --run src/Chat.test.tsx --reporter=dot` → 48 passed, EXIT 0; `uv run python -m pytest monocle/tests/test_api.py -k "capture_workbench" -x --tb=short -q` → 3 passed, EXIT 0; `cd frontend && npm run test -- --run src/useChat.test.ts --reporter=dot` → 35 passed, EXIT 0; `uv run python -m pytest monocle/tests/test_api.py -k "ingest_review_flow_supports or answering_last_question_without_actions" -x --tb=short -q` → 2 passed, EXIT 0; `uv run python -m pytest monocle/tests/test_api.py -k "capture_workbench or ingest_review" -x --tb=short -q` → 13 passed, EXIT 0; `cd frontend && npm run test -- --run src/Chat.test.tsx src/useChat.test.ts --reporter=dot` → 83 passed, EXIT 0; `cd frontend && node .\node_modules\typescript\bin\tsc --noEmit -p tsconfig.json` → EXIT 0
...
- Completed M36 end to end: added persisted ingest-review orchestration, dedicated review APIs, editable proposal mutations, contradiction hydration, deterministic diff-preview generation, and per-action plus approve-all review-state transitions that stop at `approved_pending_execution`
- Added the dedicated `/ingest-review` frontend workspace with prepared-session rail, review-state header, follow-up question answering, contradiction cards with linked notes, editable proposal drafts, visible diff rendering, and approve/reject controls; updated the app shell so prepared-session entry points route there instead of using the inbox drawer as the primary workflow
- Added focused M36 coverage in `monocle/tests/test_ingest_sessions.py`, `monocle/tests/test_api.py`, and `frontend/src/IngestReview.test.tsx`; regenerated `openapi.json` and `frontend/src/api/schema.d.ts`
- Archived M36 details to `docs/milestones.md`, marked M36 complete in `docs/build-plan.md`, and validated the full milestone: `uv run python -m pytest monocle/tests/ -x --tb=short -q` → 973 passed, 8 deselected; `cd frontend && npm run test -- --run` → 449 passed; `cd frontend && npx tsc --noEmit` → EXIT 0
- Hardened `monocle/services/ingest_prepare.py` by blocking resolved source archive paths that escape `sources_root`, so tampered `archive_path` values in the ingest-session DB now fail the prepare job instead of reading arbitrary files from disk
- Aligned ingest preparation prompt loading with the shared prompt convention by using `monocle.prompts.load_prompt("ingest_prepare")`, which restores `prompts/local/ingest_prepare.md` override support and strips YAML frontmatter before sending the system prompt to the model; also fixed `prompts/ingest_prepare.md` so its example JSON is valid
- Added focused regressions in `monocle/tests/test_ingest_prepare.py`; validation: `uv run python -m pytest monocle/tests/test_ingest_prepare.py -x --tb=short -q` → 5 passed, EXIT 0
- Closed the follow-up M36 review findings: individual per-action approvals now transition to `approved_pending_execution` when all actions are resolved and at least one is approved; rejected-only sessions no longer get promoted by `approve-all`; edited `target_file_path` values are normalized through vault path validation before persistence; diff previews now preserve explicit field clearing instead of silently falling back to the old content
- Hardened the review UI against stale detail overwrites by ignoring out-of-order `getIngestSession()` responses after rapid session switching, and expanded `frontend/src/IngestReview.test.tsx` to cover per-action approve/reject wiring plus the rapid-switch race
- Added focused regressions in `monocle/tests/test_api.py` for individual approval handoff, rejected-only `approve-all`, traversal rejection, and explicit-clear diffs; validation: `uv run python -m pytest monocle/tests/test_api.py -k ingest_review -x --tb=short -q` → 4 passed, `uv run python -m pytest monocle/tests/test_ingest_sessions.py -x --tb=short -q` → 8 passed, `cd frontend && npm run test -- --run src/IngestReview.test.tsx` → 2 passed, `cd frontend && npx tsc --noEmit` → EXIT 0
- Tightened the M36 patch API boundary by removing `approval_state` from `ProposedActionPatchRequest`, forbidding unknown fields on that request model, and narrowing `target_note_type` to the canonical note-type literal set on both backend and frontend client types
- Added focused API regressions in `monocle/tests/test_api.py` proving `PATCH /api/ingest/sessions/{id}/actions/{id}` now returns 422 for client-supplied `approval_state` and invalid `target_note_type`; refreshed `openapi.json` and `frontend/src/api/schema.d.ts`
...
- Completed M38 fast-capture end to end: surfaced `fast_capture` on persisted ingest sessions, added synchronous fast-capture orchestration over the existing prepare/review/execute services, and preserved provenance plus validation without introducing a second write path
- Exposed fast capture as a separate VoiceModal action that auto-executes clean sessions and routes blocker cases into `/ingest-review`, while labeling fast-capture fallback sessions in the review workspace
- Refreshed `openapi.json` and `frontend/src/api/schema.d.ts`, added focused backend/frontend regressions, archived M38 in `docs/milestones.md`, and updated `docs/build-plan.md`; validation: 994 backend tests passed, 457 frontend tests passed, frontend type-check EXIT 0
